# 隧道工程课程智能体 — 后端服务规范 v1.0

> 文档日期：2026-04-18
> 代码位置：`courseagent/backend/`
> 状态：独立项目，19 个 MVP 测试通过
...
**v1.0 变更：** 项目完全独立，内联 03_rag（17 文件）和 04_grading（7 文件）全部核心代码，无外部 sys.path 引用，使用 uv 独立 venv。
...
├──────────┴──────────┴───────────────────┤
│           服务层 (services/)             │
│  RAGService ─────→ services/rag/ 本地    │
│  GradingService ─→ services/grading/ 本地│
├─────────────────────────────────────────┤
...
#### GET /teacher/assignments/{id}/export — 导出成绩 CSV

```
// Response 200 → text/csv (UTF-8 BOM)
// Content-Disposition: attachment; filename*=UTF-8''assignments_1.csv

学号,姓名,班级,提交时间,总分,...,总分
S001,王同学,隧道一班,2026-04-18 14:00:00,85.5,...,85.5
```
...
│       ├── rag_service.py              # 封装本地 PipelineManager
│       ├── rag/                        # 内联 03_rag 核心组件
│       │   ├── __init__.py
...
│       │       ├── detector.py         # 结构检测
│       │       ├── tree_merge.py       # 树形合并
│       │       ├── laws_chunker.py     # 法规分块
│       │       ├── naive_chunker.py    # 通用分块
│       │       └── mineru_adapter.py   # MinerU 输出适配
│       └── grading/                    # 内联 04_grading 核心组件
│           ├── __init__.py
│           ├── models.py               # 评分模型
│           ├── agent.py                # GradingAgent
│           ├── scorer.py               # 评分计算
│           ├── rag_bridge.py           # RAG 检索桥接
│           ├── service.py              # GradingService + DB 持久化
│           └── prompts/
│               ├── dimension_score.md
│               ├── regulation_check.md
│               └── summary.md
...
| 变更 | 说明 |
|------|------|
| 项目独立 | 从 `pipelines/05_backend/` 迁移到 `courseagent/backend/`，无外部 sys.path 引用 |
| 包名统一 | `server/` → `app/`，03_rag 和 04_grading 代码内联到 `app/services/` |
| 内联 RAG | 03_rag 17 个文件 → `app/services/rag/` |
| 内联评分 | 04_grading 7 个文件 → `app/services/grading/` |
| 文件上传 | 学生提交支持 PDF/Word 文件上传 + 文本输入 |
| 附件管理 | 新增 `attachment_path` 字段 + 下载接口 |
| CSV 导出 | 教师可导出作业成绩为 CSV（UTF-8 BOM） |
| 结构化引用 | QA 答案 sources 包含 `source_name` + `chunk_index` |
| Markdown 输出 | QA answer 支持 Markdown 格式渲染 |
